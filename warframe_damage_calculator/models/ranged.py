...
class Ranged[TWeaponState: RangedState](Weapon[TWeaponState]):
    ...
    def _compute_moded_stats(self) -> None:
        ...
        self.moded.weakpoint_damage = max(self.base.weakpoint_damage + self.config.weakpoint_damage, 1)
        self.moded.multiplicative_fire_rate = max(1 + self.config.multiplicative_fire_rate, 1)
        self.moded.fire_rate = max(self.base.fire_rate * (1 + self.config.fire_rate), 0.05)
        self.moded.charge_time = self.base.charge_time / max(1 + self.config.fire_rate, 0.01)
        self.moded.reload_speed = self.base.reload_speed / max(1 + self.config.reload_speed, 0.01)
        self.moded.magazine_capacity = max(true_round(self.base.magazine_capacity * (1 + self.config.magazine_capacity)), 1)
        self.moded.ammo_efficiency = clamp(self.config.ammo_efficiency, 0, 1)
        self.moded.multishot = max(self.base.multishot * (1 + self.config.multishot), 1)
        self.moded.multiplicative_weakpoint_crit_chance = max(1 + self.config.multiplicative_weakpoint_crit_chance, 1)
        self.moded.weakpoint_crit_chance = max(self.base.crit_chance * (1 + self.config.crit_chance + self.config.weakpoint_crit_chance), 0)
        self.moded.internal_bleeding = max(self.config.internal_bleeding * (2 if max(self.moded.fire_rate * self.moded.multiplicative_fire_rate, 0.05) < 2.5 else 1), 0)

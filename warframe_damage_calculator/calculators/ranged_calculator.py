...
class RangedCalculator[TRangedState: RangedState](WeaponCalculator[TRangedState]):
    ...
    def _compute_moded_stats(self) -> None:
        ...
        self.moded.multiplicative_fire_rate = 1 if self.build.fire_rate_lock else max(1 +  self.build.multiplicative_fire_rate, 1)
        ...
        self.moded.burst_delay = max(self.base.burst_delay, 0) / (1 if self.build.fire_rate_lock else max(1 + self.build.fire_rate, 1))
        self.moded.charge_time = max(self.base.charge_time, 0) / (1 if self.build.fire_rate_lock else max(1 + self.build.fire_rate, 0.01))
        ...
        self.moded.multishot = max(self.base.multishot * (1 if self.build.multishot_lock else (1 + self.build.multishot)), 1)

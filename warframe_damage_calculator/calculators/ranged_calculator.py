...
class RangedCalculator(WeaponCalculator):
    ...
    def _compute_average_stats(self) -> None:
        ...
        related_dot = sum(
            (
                sum(multiplier * state.damage.get(damage_type) * state.damage.weight(damage_type) for damage_type, multiplier in DOT_MULTIPLIERS) * state.status_chance
                + sum(multiplier * state.forced_procs.get(damage_type) * state.damage.get(damage_type) for damage_type, multiplier in DOT_MULTIPLIERS)
            )
            * (1 + state.crit_chance * (state.crit_damage - 1))
            * state.status_damage
            * state.faction_damage ** 2
            * state.multishot
            for state in self.related.values()
        )

...
def remaining_armor(statuses: dict[str, float]) -> float:
    ...
    return (1 - corrosive_strip) * (1 - heat_strip)
...
def defense_multiplier(enemy: Any | None, kind: str, *, dot: bool, statuses: dict[str, float]) -> float:
    ...
    overguard_taken = 0.0 if dot else (1.5 if kind == "void" else 1.0) * magnetic_taken
    return (health_share * health_taken + shield_share * shield_taken + overguard * overguard_taken) / total


def damage_multiplier(enemy: Any | None, kind: str, *, zone: HitZone, dot: bool = False, weakpoint_bonus: float = 0, status_effects: dict[str, float] | None = None) -> float | None:
    ...
    return modifier * defense_multiplier(enemy, kind, dot=dot, statuses=status_effects or {}) * part


def damage_total(damage: Dist, enemy: Any | None, *, zone: HitZone, dot: bool = False, weakpoint_bonus: float = 0, status_effects: dict[str, float] | None = None) -> float | None:
    if bodypart_multiplier(enemy, zone, weakpoint_bonus) is None: return None
    return sum(amount * float(damage_multiplier(enemy, kind, zone=zone, dot=dot, weakpoint_bonus=weakpoint_bonus, status_effects=status_effects) or 0) for kind, amount in damage.items())

...
from ..domain.results import AttackResult, AverageAttackStats, BaseAttackStats, DensityMetrics, EffectiveAttackStats, FinalAttackStats, ModdedAttackStats, ResolvedStats, Stats
...
from .protocols import AttackProtocol, UpgradeProtocol, WeaponProtocol
...
def _runtime_evolution_effects(weapon: WeaponProtocol) -> tuple[ResolvedEffect, ...]:
...
def _base_damage(weapon: WeaponProtocol, attack: AttackProtocol, evolutions: ResolvedStats) -> tuple[Dist, Dist]:
...
def _damage(weapon: WeaponProtocol, attack: AttackProtocol, base: Dist, original: Dist, build: ResolvedStats, evolutions: ResolvedStats) -> Dist:
...
def _dot_base_damage(attack: AttackProtocol, base: Dist, original: Dist, build: ResolvedStats, evolutions: ResolvedStats) -> float:
    total = _combined(build, evolutions)
    common = max(1 + attack.stats.damage_bonus + float(total.proportional.get("damage_bonus", 0)), 0)
    status_bonus = family_bonus(total, "unique_status", "damage_bonus")
...
def _stance(weapon: WeaponProtocol) -> UpgradeProtocol | None:
    return next((upgrade for upgrade in weapon.build if upgrade.slot == "stance"), None)


def _stance_combo(weapon: WeaponProtocol, attack: AttackProtocol) -> Mapping[str, Any] | None:
...
def _ranged_rate(weapon: WeaponProtocol, attack: AttackProtocol, total: ResolvedStats, multishot: float) -> tuple[float, float, Stats]:
...
def _melee_rate(weapon: WeaponProtocol, attack: AttackProtocol, total: ResolvedStats) -> tuple[float, Stats]:
...
def _provisional(weapon: WeaponProtocol, attack: AttackProtocol, build_effects: tuple[ResolvedEffect, ...], evolution_effects: tuple[ResolvedEffect, ...]) -> tuple[Stats, StatusModel]:
...
def _resolve_effects(weapon: WeaponProtocol, attack: AttackProtocol, source: tuple[ResolvedEffect, ...], provisional: Stats, model: StatusModel, equipped: set[str]) -> tuple[list[ResolvedEffect], list[ResolvedEffect]]:
...
def _forced_procs(attack: AttackProtocol, effects: Iterable[ResolvedEffect]) -> Dist:
...
def _faction_factor(weapon: WeaponProtocol, total: ResolvedStats) -> float:
...
def _is_aoe_attack(attack: AttackProtocol) -> bool:
    return bool(attack.aoe or attack.category in SLAM_CATEGORIES)


def _spatial_falloff(attack: AttackProtocol, effective: EffectiveAttackStats) -> tuple[float, float | None, float | None]:
...
def _zone_damage(weapon: WeaponProtocol, result: AttackResult, zone: str, *, direct_hits: float, dot_multishot: float | None = None, damage_factor: float = 1, duplicate_multiplier: float = 1, combo_multiplier: float = 1) -> tuple[float, float] | None:
    effective, average = result.effective, result.average
    weakpoint_bonus = float(effective.weakpoint_damage_bonus)
    direct_total = damage_total(effective.damage * damage_factor, weapon.target, zone=zone, weakpoint_bonus=weakpoint_bonus, status_effects=result.status_effects, overguard_multiplier=float(effective.overguard_damage_multiplier))
    ...
    dot = _dot_value(weapon, result, zone, multishot=dot_multishot, damage_factor=damage_factor) * combo_multiplier
    return direct, dot
...
def _apply_position_mixture(weapon: WeaponProtocol, result: AttackResult, effects: list[ResolvedEffect]) -> None:
    ...
    for events, weight in weights:
        ...
        for zone, fields in ZONE_FIELDS.items():
            damage = _zone_damage(weapon, result, zone, direct_hits=multishot, dot_multishot=multishot, damage_factor=damage_factor)
    ...
    _refresh_density(density, average.sustained_fire_rate)


def _calculate_attack(weapon: WeaponProtocol, attack: AttackProtocol, build_effects: tuple[ResolvedEffect, ...], evolution_effects: tuple[ResolvedEffect, ...], *, automatic_model_override: StatusModel | None = None, status_effects_override: dict[str, float] | None = None, random_proc_probability: float = 0) -> AttackResult:
    ...
    result = AttackResult(attack.name, attack, base, modded, effective, build, evolutions, average, FinalAttackStats(**{name: deepcopy(getattr(average, name)) for name in average.__dataclass_fields__}), density, status_effects, list(attack.children), original)
    combo_multiplier = 1
    if heavy:
    ...
    for zone, fields in ZONE_FIELDS.items():
        zone_damage = _zone_damage(weapon, result, zone, direct_hits=1 if weapon.type == "melee" else multishot, duplicate_multiplier=duplicate_multiplier, combo_multiplier=combo_multiplier)
...
class AttackCalculator:
    __slots__ = ("weapon", "build_effects", "evolution_effects")

    def __init__(self, weapon: WeaponProtocol, build_effects: tuple[ResolvedEffect, ...], evolution_effects: tuple[ResolvedEffect, ...]) -> None:
        self.weapon = weapon
        self.build_effects = build_effects
        self.evolution_effects = evolution_effects

    def calculate(self, attack: AttackProtocol, *, automatic_model: StatusModel | None = None, status_effects: dict[str, float] | None = None, random_proc_probability: float = 0) -> AttackResult:
        return _calculate_attack(self.weapon, attack, self.build_effects, self.evolution_effects, automatic_model_override=automatic_model, status_effects_override=status_effects, random_proc_probability=random_proc_probability)

...
def _base_damage(context: CalculationContext, attack: Attack, perks: ResolvedStats) -> tuple[Dist, Dist, Dist]:
    ...
    raw = Dist(displayed)
    conversion = sum(float(bucket.get("impact_to_puncture_conversion", 0)) for bucket in (perks.proportional, perks.base, perks.flat))
    if conversion > 0:
    ...
    original = Dist(raw)
    flat = float(perks.base.get("damage", 0))
    if flat:
...
def _damage(attack: Attack, base: Dist, original: Dist, upgrades: ResolvedStats, perks: ResolvedStats, progenitor: Progenitor | None = None) -> Dist:
...
def _dot_base_damage(attack: Attack, base: Dist, original: Dist, upgrades: ResolvedStats, perks: ResolvedStats) -> float:
    total = _combined(upgrades, perks)
    common = max(1 + attack.stats.damage_bonus + float(total.proportional.get("damage_bonus", 0)), 0)
    status_bonus = family_bonus(total, "unique_status", "damage_bonus")

...
from ..fields.enemy_data import FACTION_TAGS, EnemyStats
...
HEALTH_SCALING: dict[str, Scaling] = {faction: STANDARD_HEALTH_SCALING for faction in FACTION_TAGS} | {
    "grineer": (0.015, 2.12, 10.733, 0.72),
    "corpus": (0.015, 2.12, 13.416, 0.55),
    "infested": (0.0225, 2.12, 16.1, 0.72),
    "orokin": (0.015, 2.1, 10.733, 0.685),
}
SHIELD_SCALING: dict[str, Scaling] = {faction: STANDARD_SHIELD_SCALING for faction in FACTION_TAGS} | {
...
class EnemyCalculator:
    ...
    def resolve(self) -> None:
        ...
        health_multiplier = level_multiplier(delta, HEALTH_SCALING.get(data.faction, STANDARD_HEALTH_SCALING))
        shield_multiplier = level_multiplier(delta, SHIELD_SCALING.get(data.faction, STANDARD_SHIELD_SCALING))

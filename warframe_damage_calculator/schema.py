...
def validate_database(database: dict[str, Any]) -> None:
    ...
    if database.get("schema_version") != 18: raise ValueError("schema version 18 is required")
    ...
    upgrade_categories = {"mod", "arcane", "perk"}
    ...
    for name, perk in database["upgrades"]["perk"].items():
        if not isinstance(perk, dict) or set(perk) - {"name", "description", "stats", "implementation_status"}: raise ValueError(f"upgrades.perk.{name}: invalid fields")
        if perk.get("name") != name: raise ValueError(f"upgrades.perk.{name}: invalid name")
        _implementation_status(perk.get("implementation_status"), f"upgrades.perk.{name}.implementation_status")
        _effects(perk.get("stats", {}), f"upgrades.perk.{name}.stats", placeholders=True)
    weapon_categories = {"primary", "secondary", "melee", "archgun"}
        ...
        for name, weapon in weapons.items():
            ...
            for tier, choices in weapon.get("evolutions", {}).items():
                for choice, record in choices.items():
                    ...
                    if perk_name not in database["upgrades"]["perk"]: raise ValueError(f"{path}: unknown perk {perk_name!r}")
                    values = record.get("values")
                    templates = database["upgrades"]["perk"][perk_name].get("stats", {})
    ...
    for section, expected_slots in (("mod", {"regular_mod", "exilus_mod", "stance_mod"}), ("arcane", {"regular_arcane"})):

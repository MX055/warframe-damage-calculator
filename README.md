...
modules (`warframe_damage_calculator.core`, engine, and loader) for advanced
...
print(weapon.results.shapley_contributions())
print(weapon.results.removal_contributions())
print(weapon.results.shapley_contributions("total_dph"))
print(weapon.results.removal_contributions("flat_dps"))
...
build. They sum to 1; interactions between upgrades mean they are attribution
weights, not independent multipliers.

Removal values are leave-one-out instead: full-build metric minus the metric with
only that upgrade removed. They do not sum to the same total when upgrades
interact. `weapon.format.upgrades()` prints both for each equipped upgrade using
`total_dps`.
...
- `damage_bonus`
- `condition_overload`
- `status_effect_stacks` (deferred Cascadia/Frostbite-style bonuses from sustained procs of one status type)
- `corpus_damage`
- `grineer_damage`
- `infested_damage`
- `murmur_damage`
